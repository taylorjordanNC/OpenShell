...
The examples use `my-sandbox` as the sandbox name.
...
A sandbox's policy has two views. The base policy is the policy you manage for
the sandbox. The effective policy is the one the sandbox enforces. It combines
the base policy with rules from attached providers, or it is the gateway-global
policy when one is active.
...
- To edit the policy, start from the base view. The effective view also contains
  rules that attached providers manage. Those rules are not part of the
  sandbox's own policy, so leave them out of the file you edit.
- To find out why a request is allowed or denied, use the effective view.
  Several rules, including provider rules, can allow the same request, and only
  the effective view shows all of them.
